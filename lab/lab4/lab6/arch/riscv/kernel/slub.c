...
void *cache_create(const char *name, size_t size, unsigned int aligns,
  ...
  puti(aligns);
  puts("\n");
//   printf("[S] Create New cache: name:%s    	size: %llu, align: %#08u", name,size,aligns );
  return s;
}
...
void *kmem_cache_alloc(struct kmem_cache *cache) {
  ...
  puts("[S] kmem_cache_alloc: name: ");
  puts(cache->name);
  puts("\naddr: ");
  puti64(object);
  puts(", partial_obj_count: ");
  puti((ADDR_TO_PAGE(object)->header)->count);
  puts("\n");
