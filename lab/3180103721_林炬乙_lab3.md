...
答：a0是当前结构的开始, a1是下一个进程的地址.  a3和a4分别是当前的和下一个进程的进程状态段数据结构地址, 40bytes 是struct 前面部分 size的大小.
...
| ------------------------------------ | -------------------------- | -------- | -------- |
| 不同优先级                           | {1,1},{2,1},{3,1},{4,1}    | 4>3>2>1  |          |
| 相同优先级, 不同剩余时间             | {1,4},{1,3},{1,2},{1,1}    | 4>3>2>1  |          |
| 相同优先级和剩余时间，以遍历顺序执行 | {1,1},{1,1},{1,1},{1,1}    | 4>3>2>1  |          |
|                                      |                            |          |          |
